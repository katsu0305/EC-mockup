...
> 注記
> 本文は設計時の手順書です。現行実装では商品データ取得元は ECDB API ではなく MakeShop GraphQL API に移行しています。

## 1. 目的

GitHub、MakeShop API、MakeShop 画像、実ショップ参照の 4 系統から必要データを取得できるようにする。
...
1. MakeShop API の接続確認を行う
2. `searchProduct` による商品一覧取得を実装する
3. `getProductCategory` によるカテゴリ一覧取得を実装する
4. 公開中商品のみを中間 JSON に保存する
